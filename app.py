...
@st.cache_data
def load_config():
    with open("config/teamcenter_config.json", "r", encoding="utf-8") as f:
        return json.load(f)
...
if "reviewed" in st.session_state:
    ...
    with tabs[2]:
        ...
        for tc in cases:
            with st.expander(f'{tc.get("test_case_id", "")} - {tc.get("title", "")}'):
                st.write(f'**Category:** {tc.get("category", "")} | **Priority:** {tc.get("priority", "")} | **Role:** {tc.get("user_role", "")}')
                st.write("**Objective:**", tc.get("objective", ""))
                st.write("**Preconditions:**")
                for x in tc.get("preconditions", []): st.write("-", x)
                st.write("**Steps:**")
                for step in tc.get("steps", []):
                    st.markdown(f'**{step.get("step_number")}. Action:** {step.get("action", "")}')
                    st.success(f'Expected: {step.get("expected_result", "")}')
                st.write("**Traceability:**", ", ".join(tc.get("traceability", [])))
    with tabs[3]:
